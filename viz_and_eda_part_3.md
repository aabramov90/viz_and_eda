...
This is a dataframe\!  
The `group by` and `summarize` are creating a nice new dataset for us to
plot.
...
<img src="viz_and_eda_part_3_files/figure-gfm/unnamed-chunk-11-1.png" width="90%" />

Suppose you want to summarize many columns in the same way. Use the
across function\!
...
Just a reminder, sometimes your data results are easier to read in
another format\!
...
So here we have grouped, and then created a new column with the mean and
now plotted the values as a difference from the mean
...
<img src="viz_and_eda_part_3_files/figure-gfm/unnamed-chunk-14-1.png" width="90%" />

What about window functions? So here we have grouped by name and then
assigned relative ranks among the tmax variable, then we looked at the
tmax of every month.
...
lag Here we are taking the values, and then it creates a new column with
the value, down a row, or however many you want. Does yesterday’s temp
predict today’s?
